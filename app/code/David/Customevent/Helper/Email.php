...
class Email extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    public function __construct(
        ...
        \David\Customevent\Mail\Template\TransportBuilder $transportBuilder
    ) {
     ...
     * @return void
     */
    public function generateTemplate($emailTemplateVariables,$senderInfo,$receiverInfo,$bcc = "",  $dataFile = array(), $replyTo = '')
    {
        $template =  $this->_transportBuilder->setTemplateIdentifier($this->temp_id)
                ...
                ->setReplyTo($replyTo);
            if(count($dataFile)){
                $template->addAttachment($dataFile['body'], $dataFile['name']);
            }
            if(!empty($bcc)){
                $template->addBcc($bcc);
            }
    ...
    public function sendMail($templateId, $emailTemplateVariables, $senderInfo, $receiverInfo, $copyTo = "", $dataFile = array(), $replyTo = '')
    {	
		$status = false;
		...
		try{
			...
			$this->generateTemplate($postObject,$senderInfo,$receiverInfo,$copyTo,$dataFile,$replyTo);
			$transport	= $this->_transportBuilder->getTransport();
			$transport->sendMessage();    
			$status		= true;	 	
		}catch(\Exception $e){
			$writer = new \Zend\Log\Writer\Stream(BP . '/var/log/sendmailqueue.log');
